...
文件类型	权限 	 引用数		所有者  所属组	 大小	修改时间			文件名	

文件类型： -普通文件：纯文本文件(ASCII)、二进制文件(scripts)、数据格式文件(如/var/log/wtmp)d		   d目录	l链接	b块(存储)设备如硬盘		c字符(输入)设备,如键盘、鼠标
...
引用数：文件的表示引用该文件硬链接数；目录的表示该目录有多少个一级目录
...
find		路径	选项	搜索内容	完全匹配的才能搜索到，模糊搜索用通配符
...
-s							更详细信息
Ip	link show					显示网络接口
Ip	link	list					显示网络设备运行状态	=ifconfig
...
配置文件
/etc/passwd				用户信息
账号(记忆):密码标志(x,删除后则无密码):UID(通过此识别用户分配权限)：GID(初始组)：用户说明:家目录：登录后的shell
忘记密码进入单用户模式删除此文件的密码标志清空密码
很多帐号是系统正常运行必须的，称系统帐号，如 bin, daemon, adm, nobody 等，不要随意杀掉 
...
用户
useradd		一般使用默认设置即可
/etc/default/useradd(useradd -D)	/etc/login.defs 默认值
passwd
...
id				查看UID、GID
finger			-s ：仅列出使用者的帐号、全名、终端机代号与登陆时间等等；
	su				切换身份
...
-a			添加进组
-d			从组中删除
usermod -G group user		gpasswd -a user group
newgroup	改变有效组
groups		有效组与支持组查询
...
du	[目录或文件名]	统计目录或文件占磁盘空间大小	usage
...
SWAP分区
	fdisk		/dev/sdb		等命令建立swap分区		编号82
	mkswap	/dev/sdb1	格式化swap分区
	swapon	/dec/sdb1	加入swap分区	取消swapoff
...
服务启动的分类：
		...
		总管程序（super daemon）：由特殊的xinetd或inetd这两个总管程序提供 									socket 对应或port对应的管理
当没有用户要求某socket或port时，所需要的服务是不会被启动的。
若有用户要求时，xinetd 总管才会去唤醒相对应的服务程序。
当该要求结束时，这个服务也会被结束掉。
因为通过 xinetd所总管，被被称为super daemon。
...
status ：目前后面接的这个unit的详细状态，是否正在执行、开机默认执行、登录
show  :显示默认设置
...
mask		这个daemon无论如何都无法被启动,已被强制注销(非删除)，强力disable
可通过 systemctl unmask 方式改回原本状态
...
[Service], [Socket], [Timer], [Mount], [Path]：不同的unit type就得要使用相对应的设置项目。					主要规范服务启动的脚本、环境配置文件文件名、重新启动的方式等
...
设置参数需要有“是/否”的项目（布林值, boolean）
				可以使用 1, yes, true, on 代表启动，用 0, no, false, off 代表关闭
空白行、开头为 # 或 ; 的代表注解
